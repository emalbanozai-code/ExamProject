...
def recalculate_member_cycle(cycle_id: int) -> MemberFeeCycle:
...
def get_or_create_staff_period(staff: Staff, period_month: date) -> StaffSalaryPeriod:
    ...
    period, _ = StaffSalaryPeriod.objects.get_or_create(
        staff=staff,
...
def recalculate_staff_period(period_id: int) -> StaffSalaryPeriod:
    with transaction.atomic():
        ...
        paid_amount = Decimal(aggregates["total_paid"])
        remaining_amount = period.gross_salary_amount - paid_amount
        if remaining_amount < ZERO:
        ...
        period.status = _staff_period_status_from_remaining(
            period.gross_salary_amount, remaining_amount
        )
        period.save(update_fields=["paid_amount", "remaining_amount", "status", "updated_at"])
